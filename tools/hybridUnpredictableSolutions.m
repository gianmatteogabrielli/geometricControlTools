function [Pi_x, Pi_w, A_x, A_w, E_x, E_w, Omega, S_r, J_r, Gamma_x, Gamma_w, Lambda_x, Lambda_w] = hybridUnpredictableSolutions(A, B, E, F, P, R, S, J, K, K_d, V, control)
    Pi_x = V(1:end-size(S,2),1:end-size(S,2));
    Pi_w = V(1:end-size(S,2), end-size(S,2) +1:end);
    S_r = S;
    J_r = J;
    n = size(A, 2);
    q = size(S, 2);
